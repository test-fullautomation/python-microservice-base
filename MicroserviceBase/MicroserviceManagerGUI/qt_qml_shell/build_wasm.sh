#!/bin/bash
# build_wasm.sh — Build the QML Shell as a WASM binary.
#
# Prerequisites:
#   - Emscripten SDK (emsdk) activated in PATH
#   - Qt 6.x built for WASM (qt-wasm prefix)
#
# Usage:
...
echo "=== Build complete ==="
echo "Output files:"
ls -lh qtshell.js qtshell.wasm 2>/dev/null || echo "(check build directory for output)"

# --- Deploy to web/qt-shell/ ---
DEPLOY_DIR="${SCRIPT_DIR}/../web/qt-shell"
echo ""
echo "=== Deploying to web/qt-shell/ ==="
mkdir -p "$DEPLOY_DIR"
cp qtshell.js   "$DEPLOY_DIR/"
cp qtshell.wasm "$DEPLOY_DIR/"
