...
:: ===================================================================
:: build_wasm.bat — Build the QML Shell WASM binary and deploy
:: ===================================================================
::
:: Usage:
...
:: Output:
::   ../web/qt-shell/
::   ├── qtshell.wasm     WASM binary
::   └── qtshell.js       Emscripten JS loader
:: ===================================================================
...
set "DEPLOY_DIR=!SCRIPT_DIR!\..\web\qt-shell"
...
:: ----- Configure -----
echo.
echo ===== Configuring QML Shell WASM Build [!BUILD_TYPE!] =====
echo   Qt WASM: %QT_WASM_DIR%
echo   Qt Host: %QT_HOST_DIR%
echo.

:: Emscripten requires Ninja (not Visual Studio generator).
:: We call cmake directly instead of qt-cmake.bat for full control over
:: the generator and toolchain options.
...
:: ----- Build -----
echo.
echo ===== Building QML Shell WASM =====

:: If .js exists but .wasm is missing (e.g. deleted externally), ninja thinks
:: the build is up-to-date. Delete the .js to force a relink.
if exist "!BUILD_DIR!\qtshell.js" if not exist "!BUILD_DIR!\qtshell.wasm" (
    echo   .wasm missing but .js exists — forcing relink...
    del "!BUILD_DIR!\qtshell.js" 2>nul
)
...
:: ----- Deploy to web/qt-shell/ -----
echo.
echo ===== Deploying to web/qt-shell/ =====

:: Find output files (Ninja: directly in build dir, VS: in Release/ subdir)
set "WASM_DIR=!BUILD_DIR!"
if exist "!BUILD_DIR!\!BUILD_TYPE!\qtshell.wasm" set "WASM_DIR=!BUILD_DIR!\!BUILD_TYPE!"

if not exist "!WASM_DIR!\qtshell.wasm" (
    echo ERROR: qtshell.wasm not found in !WASM_DIR!
...
del "!DEPLOY_DIR!\qtshell.wasm" 2>nul
del "!DEPLOY_DIR!\qtshell.js" 2>nul
copy /Y "!WASM_DIR!\qtshell.wasm" "!DEPLOY_DIR!\" >nul
echo   Copied qtshell.wasm
copy /Y "!WASM_DIR!\qtshell.js" "!DEPLOY_DIR!\" >nul
echo   Copied qtshell.js

:: ----- Summary -----
echo.
echo ===== QML Shell WASM build complete =====
echo.
echo   Output: web/qt-shell/
echo     qtshell.wasm
echo     qtshell.js
echo.
