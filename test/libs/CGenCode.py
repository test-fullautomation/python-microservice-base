...
class CGenCode():
   ...
   def GenCode(self):
      ...
      PYTESTLISTFILE_RST = f"{REFERENCEPATH}/MSB_TestUsecases.rst"
      oPyTestListFile_RST = CFile(PYTESTLISTFILE_RST)
      print(f"* '{PYTESTLISTFILE_RST}'")

      PYTESTLISTFILE_TXT = f"{REFERENCEPATH}/MSB_TestUsecases.txt"
      oPyTestListFile_TXT = CFile(PYTESTLISTFILE_TXT)
      print(f"* '{PYTESTLISTFILE_TXT}'")

      PYTESTLISTFILE_CSV = f"{REFERENCEPATH}/MSB_TestUsecases.csv"
      oPyTestListFile_CSV = CFile(PYTESTLISTFILE_CSV)
      print(f"* '{PYTESTLISTFILE_CSV}'")

      PYTESTLISTFILE_HTML = f"{REFERENCEPATH}/MSB_TestUsecases.html"
...
<meta http-equiv="content-type" content="text/html; charset=windows-1252">
   <meta name="MicroserviceBase" content="MicroserviceBase">
   <title>MicroserviceBase Test Cases</title>
</head>
...
<b>
MicroserviceBase Test Cases<br>
(<em>gRPC + Consul + Nomad scaffold generator</em>)
</b></font>
</div>
<hr width="100%" color="#FF8C00" align="center">
...
"""
      ...
      for dictUsecase in listofdictUsecases:
         ...
         # prepare web server path to test file (will be a link inside the HTML use case list)
         ...
         TESTFILElink   = f"https://github.com/test-fullautomation/python-microservice-base/blob/develop/test/testfiles/{TESTFILEname}"
...
<b>
<a target="_blank" href="####TESTFILELINK####">####TESTID####</a>
</b></font></td>
...
"""
         ...
         sOut = sOut.replace('####TESTFILELINK####', TESTFILElink)
         ...
         if COMMENT is None:
            sOut = sOut.replace('####COMMENT####', "")
         else:
            sReplace = f"<br>\n{COMMENT}"
            sOut = sOut.replace('####COMMENT####', sReplace)
         if HINT is None:
...
"""
      ...
      for sKey in dictPyTestFiles:
         ...
         for dictUsecase in listofdictUsecasesInFile:
            ...
            DESCRIPTION = dictUsecase['DESCRIPTION']
            EXPECTATION = dictUsecase['EXPECTATION']
            if "COMMENT" in dictUsecase and dictUsecase["COMMENT"] is not None:
               oPyTestFile.Write(f"   # ({dictUsecase['COMMENT']})")
            if "HINT" in dictUsecase and dictUsecase["HINT"] is not None:
               oPyTestFile.Write(f"   # ({dictUsecase['HINT']})")
