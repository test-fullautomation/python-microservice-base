...
:: ===================================================================
:: build_wasm.bat — Build the Qt WASM GUI and copy output to GUIs/
:: ===================================================================
::
:: Usage:
...
if exist "!BUILD_DIR!\myqtwasmservice.js" if not exist "!BUILD_DIR!\myqtwasmservice.wasm" (
    echo   .wasm missing but .js exists — forcing relink...
    del "!BUILD_DIR!\myqtwasmservice.js" 2>nul
)
...
if exist "!BUILD_DIR!\!BUILD_TYPE!\myqtwasmservice.wasm" set "WASM_DIR=!BUILD_DIR!\!BUILD_TYPE!"

if not exist "!WASM_DIR!\myqtwasmservice.wasm" (
    echo ERROR: myqtwasmservice.wasm not found in !WASM_DIR!
    echo        Check build output above for errors.
    exit /b 1
)

:: Delete existing files first to avoid Windows case-insensitive overwrite
:: retaining a stale mixed-case filename (e.g. MyQtWasmService.js).
del "!SCRIPT_DIR!\GUIs\myqtwasmservice.wasm" 2>nul
del "!SCRIPT_DIR!\GUIs\myqtwasmservice.js" 2>nul
copy /Y "!WASM_DIR!\myqtwasmservice.wasm" "!SCRIPT_DIR!\GUIs\" >nul
echo   Copied myqtwasmservice.wasm
copy /Y "!WASM_DIR!\myqtwasmservice.js" "!SCRIPT_DIR!\GUIs\" >nul
