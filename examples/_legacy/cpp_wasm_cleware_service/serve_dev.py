#!/usr/bin/env python3
"""
serve_dev.py — Dev server for ServiceCleware Qt WASM GUI.

Serves the WASM build output AND provides an HTTP->RabbitMQ bridge so the
Qt WASM GUI can call the running ServiceCleware.exe backend.
...
def find_build_dir(script_dir):
    for sub in ["build/wasm", "build/Qt_WASM-Release", "build"]:
        ...
        if os.path.isfile(os.path.join(d, "servicecleware_wasm.wasm")):
            return d
    return None
...
def main():
    ...
    print("=" * 60)
    print("  ServiceCleware WASM Dev Server")
    print("=" * 60)
    print(f"  Serving:  {build_dir}")
    print(f"  URL:      http://localhost:{port}/servicecleware_wasm.html")
