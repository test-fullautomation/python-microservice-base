// Console client for MultiProto (multi-proto).
//
// All services are hosted by ONE binary on ONE port, so we make ONE
// channel and instantiate all per-service stubs on it.
//
// Usage:
//   multi_proto_client                    # discover host:port via Consul (default)
//   multi_proto_client --direct HOST:PORT # bypass Consul, connect directly
...
int main(int argc, char** argv) {
    ...
    if (!direct) {
        ...
        const char* consul_addr_env = std::getenv("MULTI_PROTO_CONSUL_ADDR");
        ...
        auto endpoint = resolver.resolve("multi_proto");
        if (endpoint.empty()) {
            std::cerr << "Consul lookup for 'multi_proto' returned no instances. "
    ...
    std::cout << "Connecting to MultiProto @ " << target << std::endl;
