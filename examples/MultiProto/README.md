...
A demo sample for generating service from multi proto files.
...
│   ├── Settings.h               # shared config (env prefix MULTI_PROTO_)
...
├── deploy/multi_proto.nomad.hcl  # Nomad job (single .exe, dynamic port)
...
flowchart TD
    ...
    BuildResult["build-qt-vcpkg/multi_proto.exe<br/>+ runtime DLLs alongside"]
    LocalRun["Run locally:<br/>multi_proto.exe"]
    Pkg["deploy_qt_vcpkg.bat<br/>→ dist-qt-vcpkg/"]
    NomadRun["nomad job run<br/>deploy/multi_proto.nomad.hcl"]
    Discover["Clients discover via Consul<br/>(svc name: multi_proto)"]
...
Output: one executable `build*/multi_proto.exe` listening on
`$MULTI_PROTO_GRPC_PORT` (Nomad-allocated when run under Nomad).
