// Console client for TestService (multi-proto).
//
// All services are hosted by ONE binary on ONE port, so we make ONE
// channel and instantiate all per-service stubs on it.
//
// Usage:
//   test_service_client                    # discover host:port via Consul (default)
//   test_service_client --direct HOST:PORT # bypass Consul, connect directly
...
int main(int argc, char** argv) {
    ...
    if (!direct) {
        ...
        const char* consul_addr_env = std::getenv("TEST_SERVICE_CONSUL_ADDR");
        ...
        auto endpoint = resolver.resolve("test_service");
        if (endpoint.empty()) {
            std::cerr << "Consul lookup for 'test_service' returned no instances. "
    ...
    std::cout << "Connecting to TestService @ " << target << std::endl;
