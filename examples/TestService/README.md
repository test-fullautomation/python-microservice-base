...
TestService multi-service binary.
...
│   ├── Settings.h               # shared config (env prefix TEST_SERVICE_)
...
├── deploy/test_service.nomad.hcl  # Nomad job (single .exe, dynamic port)
...
flowchart TD
    ...
    BuildResult["build-qt-vcpkg/test_service.exe<br/>+ runtime DLLs alongside"]
    LocalRun["Run locally:<br/>test_service.exe"]
    Pkg["deploy_qt_vcpkg.bat<br/>→ dist-qt-vcpkg/"]
    NomadRun["nomad job run<br/>deploy/test_service.nomad.hcl"]
    Discover["Clients discover via Consul<br/>(svc name: test_service)"]
...
Output: one executable `build*/test_service.exe` listening on
`$TEST_SERVICE_GRPC_PORT` (Nomad-allocated when run under Nomad).
